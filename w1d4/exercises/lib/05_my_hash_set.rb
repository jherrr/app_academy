...
class MyHashSet
    ...
    def symmetric_difference(set2)
        temp = self.dup
        temp = temp.minus(set2)
        
        output = MyHashSet.new
        temp.to_a.each do |value|
            output.insert(value)
        end
        
        temp = set2.minus(self.dup)
        temp.to_a.each do |value|
            output.insert(value)
        end
    ...
    def ==(object)
        ...
        return false if self.to_a.length != object.to_a.length
        return false if self.to_a != object.to_a
        true
    end
